...
export default function UpsetUI() {
  ...
  useEffect(() => {
    ...
    // 确保数据存在且有效
    if (!selectedParticipants[0] || !Array.isArray(selectedParticipants[0])) {
      console.warn('Invalid selectedParticipants data');
      return;
    }

    const q = selectedParticipants[0].map((_, colIndex) => 
      selectedParticipants.map(row => row[colIndex])
    );
    
    const results = qmethod(q, nfactors, distribution);
    if (!results || !results.dataset || !results.zsc) return;
    ...
    // 确保 mat[0] 存在
    if (!results.zsc.length || !results.zsc[0]) return;
    ...
    const upsetData = factors.map((factor, factorIndex) => {
      const sets = txtMatrix
        .filter((_, statementIndex) => mat[statementIndex] && mat[statementIndex][factorIndex] === 1)
  ...
  return (
    <div className="mx-auto p-4">
      ...
      {/* 移除 jsx 属性，改用 className */}
      <style className="details:not([open]) .dropdown-content { display: none; }" />
    </div >
  );
}
